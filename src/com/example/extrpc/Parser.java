...
//		pu = new CommonParserUtil("grammar_rules.txt", "action_table.txt", "goto_table.txt");
		...
		pu.rule("TopLevel -> id = LExpr", () -> {
			return new TopLevel(new Var(pu.getText(1)), (Term) pu.get(3));
		});
		pu.rule("TopLevel -> id = LExpr ; TopLevel", () -> {
			return new TopLevel(new Var(pu.getText(1)), (Term) pu.get(3), (TopLevel) pu.get(5));
		});
		...
		pu.rule("LExpr -> let id = LExpr in LExpr end", () -> {
			return new Let(new Var(pu.getText(2)), (Term) pu.get(4), (Term) pu.get(6));
		});
		...
		pu.rule("LogicNot -> ! LogicOr", () -> {
			return new Logical((Term) pu.get(2), Logical.NOT);
		});
		pu.rule("LogicNot -> LogicOr", () -> {
			return pu.get(1);
		});
		pu.rule("LogicOr -> LogicOr or LogicAnd", () -> {
			return new Logical((Term) pu.get(1), Logical.OR, (Term) pu.get(3));
		});
		pu.rule("LogicOr -> LogicAnd", () -> {
			return pu.get(1);
		});
		pu.rule("LogicAnd -> LogicAnd and CompEqNeq", () -> {
			return new Logical((Term) pu.get(1), Logical.AND, (Term) pu.get(3));
		});
		...
		pu.rule("CompEqNeq -> CompEqNeq == Comp", () -> {
			return new Comp((Term) pu.get(1), Comp.EQUAL, (Term) pu.get(3));
		});
		pu.rule("CompEqNeq -> CompEqNeq != Comp", () -> {
			return new Comp((Term) pu.get(1), Comp.NOTEQUAL, (Term) pu.get(3));
		});
		pu.rule("CompEqNeq -> Comp", () -> {
			return pu.get(1);
		});
		pu.rule("Comp -> Comp < ArithAddSub", () -> {
			return new Comp((Term) pu.get(1), Comp.LTHAN, (Term) pu.get(3));
		});
		pu.rule("Comp -> Comp <= ArithAddSub", () -> {
			return new Comp((Term) pu.get(1), Comp.LEQUAL, (Term) pu.get(3));
		});
		pu.rule("Comp -> Comp > ArithAddSub", () -> {
			return new Comp((Term) pu.get(1), Comp.GTHAN, (Term) pu.get(3));
		});
		pu.rule("Comp -> Comp >= ArithAddSub", () -> {
			return new Comp((Term) pu.get(1), Comp.GEQUAL, (Term) pu.get(3));
		});
		...
		pu.rule("ArithUnary -> - Term", () -> {
			return new Arithmetic((Term) pu.get(2), Arithmetic.UNARY);
		});
		pu.rule("ArithUnary -> Term", () -> {
			return pu.get(1);
		});
